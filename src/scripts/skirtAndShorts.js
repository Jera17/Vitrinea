...
var nodes = [23, 25, 11] //Hombros-Cintura: [12, 11, 23], Hombros-Rodilla: [12, 11, 25], Hombros-Tobillo: [12, 11, 27]
...
function getCoords(rsl, nodes) {
  ...
  const x4 = (rsl[nodes[1]+1].x) //cadera derecha
  const y4 = (rsl[nodes[1]+1].y)
  ...
  const torsosHeight = (Math.sqrt(Math.pow((x2 - x1), 2) + Math.pow((y2 - y1), 2)) * 1.2)+torsosHeightAux  //largo del cadera a la rodilla
  ...
  ctx.drawImage(selectedImage, Xcenter - (sizeX/2) + newXposition, Ycenter - (torsosHeight/2) - newYposition, sizeX, torsosHeight)
}
