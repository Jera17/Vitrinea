...
var nodes = [11, 27] //Hombros-Cintura: [12, 11, 23], Hombros-Rodilla: [12, 11, 25], Hombros-Tobillo: [12, 11, 27]
...
function getCoords(rsl, nodes) {
  ...
  const torsosHeight = Math.sqrt(Math.pow((x2 - x1), 2) + Math.pow((y2 - y1), 2)) * 1.1 //largo del hombro a la cadera
  const magX = x1 - x0
  const magY = y1 - y0
